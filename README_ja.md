...
パッケージレジストリのバージョン変更を監視するローカルRust CLIツール。

現状の `pincushion check` は、最新バージョン解決・baseline/change state の保存・変更パッケージの要約表示までを実行します。artifact の fetch/unpack/diff/review/report 用モジュール自体はありますが、まだ `check` の実行経路には完全には接続されていません。
...
**現状の2回目以降**の `pincushion check` は、以下を実行します:

1. 設定された各パッケージの最新バージョンを解決
2. 保存済みベースラインと比較
3. changed / unchanged / newly tracked package を表示
4. `.pincushion/seen.json` を更新

今の実装ギャップは、`pincushion check` 自体から **fetch -> unpack -> diff** が **npm / RubyGems / PyPI / crates.io の4 ecosystem 全部**で動くまで閉じたことになりません。個別モジュールやテストだけで成立していても完了扱いにはしません。

そのギャップが埋まった後の目標パイプライン:
...
以下は、artifact diff 経路が `check` に接続された後にフルパイプラインで出したいシグナルです。
...
artifact diff 経路が `check` に接続された後は、`review.provider` で自動レビューを制御します:
...
artifact diff 経路が `check` に接続された後は、レポートは `.pincushion/reports/<ecosystem>/<package>/<old>-<new>.{json,md}` に出力されます。
...
pincushionはウォッチリスト設定と同じディレクトリに `.pincushion/` を作成します。現状で実際に使っているのは主に `seen.json` で、残りのディレクトリは artifact パイプラインを `check` に接続した後の想定レイアウトです:
...
| コード | 意味 |
|-------|------|
| 0 | partial lookup failure がない正常終了 |
